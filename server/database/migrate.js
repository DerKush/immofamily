...
async function seed() {
  ...
  const b1  = await insertBien(['Villa F5 Résidentielle',   'Cocody',  'Villa',       180,650000,'occupe', 5.356,-3.989,'Villa avec piscine.',null,'Rte des Ambassadeurs']);
  const b2  = await insertBien(['Appartement F3 Riviera',   'Riviera', 'Appartement',  90,280000,'occupe', 5.374,-3.970,'F3 lumineux.',null,'Résidence Palmiers']);
  const b3  = await insertBien(['Studio Plateau Centre',    'Plateau', 'Studio',        35,200000,'vacant', 5.318,-4.024,'Studio meublé.',null,'Av. Botreau-Roussel']);
  const b4  = await insertBien(['F2 Angré Cité',            'Angré',   'Appartement',   60,180000,'occupe', 5.385,-3.987,'F2 moderne.',null,'Cité Lauriers']);
  const b5  = await insertBien(['Local Commercial Marcory', 'Marcory', 'Local commercial',55,300000,'occupe',5.297,-3.999,'Local vitrine.',null,'Cité Kénèdougou']);

  const imm1 = await insertBien(['Immeuble Résidence Cocody','Cocody','Immeuble',null,0,'occupe',5.362,-3.991,'R+3 gardiennage.',null,'12 Av. Préfet Camille Adam']);
  ...
  const u3 = await insertBien(['Appt B1 – 1er', 'Cocody','Appartement',75,240000,'occupe',null,null,'F3 1er.',imm1,null]);
  const u4 = await insertBien(['Appt B2 – 1er', 'Cocody','Appartement',75,240000,'occupe',null,null,'F3 1er.',imm1,null]);
  const u5 = await insertBien(['Appt C1 – 2ème','Cocody','Appartement',80,260000,'occupe',null,null,'F3 2ème.',imm1,null]);
  const u6 = await insertBien(['Appt C2 – 2ème','Cocody','Studio',    42,160000,'maintenance',null,null,'Rénovation.',imm1,null]);

  const imm2 = await insertBien(['Immeuble Riviera Park','Riviera','Immeuble',null,0,'occupe',5.372,-3.968,'R+2.',null,'Bd Corniche']);
  ...
  const u9  = await insertBien(['Appt 201','Riviera','Appartement',95,340000,'occupe',null,null,'F3 terrasse.',imm2,null]);
  const u10 = await insertBien(['Appt 202','Riviera','Appartement',95,340000,'occupe',null,null,'F3.',imm2,null]);

  const insertLoc = async (v) => {
    const r = await db.query(`INSERT INTO locataires (nom,email,telephone,bien_id,date_entree,date_echeance) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, v);
    return r.rows[0].id;
  };
  ...
  const insertPay = (v) => db.query(
    'INSERT INTO paiements (locataire_id,bien_id,montant,date_paiement,mode,statut,mois,annee) VALUES ($1,$2,$3,$4,$5,$6,$7,2026)', v);
