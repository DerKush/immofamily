        ...
        {NAVITEMS.map((item, i) => (
          <div key={item.to}>
            ...
            <NavLink to={item.to} end={item.to === '/'} style={({ isActive }) => ({
              ...
              {item.label}
              {item.dot && <span style={{ width: 7, height: 7, background: 'var(--terra)', borderRadius: '50%', marginLeft: 'auto' }} />}
            </NavLink>
          </div>
        ))}
      </nav>
        ...
        <div style={{ background: 'rgba(255,255,255,.08)', borderRadius: 8, padding: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,.4)' }}>{user?.role === 'admin' ? 'Administrateur' : 'Membre'}</div>
            </div>
          </div>
